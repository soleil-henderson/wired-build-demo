-- Wired Build — Seed the parts catalogue.
--
-- ~30 popular AU 4WD parts so the Log-a-Mod autocomplete has content from
-- day one. Marked source='wired'/is_approved=true so they bypass the
-- moderation queue. Supabase migrations only run once, so the absence of
-- ON CONFLICT here is intentional.

insert into public.parts (brand, name, category, price_min, price_max, source, is_approved, install_difficulty) values
  -- Suspension
